...
이 코드에서는 탭의 역할이나 구조를 알려주는 속성이 지정되지 않았어요. 겉보기에는 탭을 위한 기본적인 골격을 갖춘 것처럼 보이지만, 스크린리더는 이를 명확하지 않은 정보로 전달하게 돼요.
...
- `role="tablist"`: 탭 버튼들을 하나의 그룹으로 묶어 스크린리더가 “탭 목록”으로 인식해요.
