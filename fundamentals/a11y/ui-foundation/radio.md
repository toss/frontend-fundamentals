...
이 코드는 `fieldset` 이나 `name` 속성을 지정하지 않은 상태예요. 겉보기에는 라디오 버튼이 구성되어 있지만, 이 코드는 라디오 버튼들이 그룹으로 묶이지 않았고, `name` 속성이 없어 같은 그룹으로 인식되지 않아요. 따라서 스크린리더는 이를 개별적인 독립된 버튼으로만 인식하게 돼요.
...
라디오 버튼을 구현할 때는 **옵션을 묶는 그룹**과 **각각 선택할 수 있는 옵션**이 필요해요.

이 두 가지를 `fieldset`, `legend`, `input[type="radio"]` 요소로 명확히 구분해주면 스크린리더가 라디오 그룹의 목적과 현재 선택 상태를 정확히 인식하고 전달할 수 있어요.
...
- 각 `input[name]`: 동일한 `name`으로 하나만 선택되도록 동작해요.
  ::::
...
아래 예제는 **배송 방법**과 **결제 방법**처럼 서로 다른 그룹의 라디오 버튼에 서로 다른 `name` 값을 지정하는 코드예요.
