...
## 모듈 시스템
하나의 파일에 모든 기능을 몰아넣으면 수정할 때도 어렵고, 협업도 불편해요.  
그래서 우리는 코드를 작은 조각들(모듈)로 나눠서 필요한 부분만 가져다 쓰는 방식, 즉 **모듈 시스템**을 사용해요.

모듈 시스템을 쓰면 이런 점이 좋아요.

- **코드 재사용성**: 기능을 모듈로 쪼개놓으면 여러 곳에서 쉽게 다시 쓸 수 있어요.
- **유지보수성**: 어디에서 문제가 생겼는지 빠르게 찾고 고칠 수 있어요.
- **캡슐화**: 모듈 내부 코드가 전역 스코프를 어지럽히지 않아서 안전해요.
- **의존성 관리**: 어떤 파일이 무엇을 쓰는지 명확하게 알 수 있어요.

## 웹팩이 모듈 시스템을 다루는 방식

요즘 브라우저는 `import`와 `export`를 지원해서 ES 모듈을 그대로 쓸 수도 있어요.  
하지만 실무에선 다음 같은 이유로 웹팩 같은 번들러를 사용해요.

1. **네트워크 요청이 많아져요**  
   모듈이 여러 개면 브라우저가 각각 요청을 보내야 해요. 많아지면 로딩 속도에 영향을 줄 수 있어요.

2. **브라우저 호환성 문제**  
   오래된 브라우저는 ES 모듈을 아예 지원하지 않기도 해요.

3. **최적화가 부족해요**  
   사용하지 않는 코드를 자동으로 제거해주지도 않고, 압축도 안 해줘요.

웹팩을 사용하면 이런 문제를 깔끔하게 해결할 수 있어요.

- **의존성 자동 관리**: 어떤 파일이 무엇을 쓰는지 분석해서 자동으로 정리해줘요.
- **코드 변환**: 최신 JS나 TypeScript도 모든 브라우저에서 쓸 수 있게 변환해줘요.
- **최적화**: 쓰지 않는 코드는 지우고, 코드도 압축해서 파일 크기를 줄여줘요.
...
3. **CDN 의존성**  
   `date-fns`를 CDN으로 불러오고 있어서 오프라인 개발이 어렵고 버전 관리도 힘들어요.

:::details 전역 스코프 노출은 왜 문제가 될까요?
- **이름 충돌**: 다른 스크립트가 같은 이름을 쓰면 덮어씌워질 수 있어요.
- **의도치 않은 수정**: 어디서든 접근되다 보니 실수로 값이 바뀌기 쉬워요.
- **의존성 불명확**: 이 값이 어떤 흐름에서 오는지 추적하기가 어려워져요.
