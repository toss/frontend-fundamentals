...
번들링 과정에서 **모듈이 실제 어떤 파일을 가리키는지를 판단하는 과정**을 **리졸브(resolve)**라고 해요. 
이 동작은 번들러 내부의 **리졸버(resolver)**가 담당하며, 설정된 규칙에 따라 경로를 결정하는 과정을 **리졸루션(resolution)**이라고 불러요.

리졸버는 `import` 또는 `require` 구문을 만나면, 다음 방식을 통해 모듈을 찾아요:
  - 1. 경로가 명시된 경우 → 파일 시스템에서 조회
  - 2. 경로가 명시되지 않은 경우 → 설치된 패키지(node_modules)에서 탐색

이러한 탐색 방식은 설정 파일의 resolve 필드에서 조정할 수 있어요. 
예를 들어 특정 디렉토리를 우선 탐색하도록 하거나, 자주 사용하는 경로에 별칭을 지정해 코드의 가독성과 유지보수성을 높일 수 있어요.
...
경로(절대/상대)를 기반하여 모듈을 임포트한 경우, 파일 시스템을 이용하여 리졸브합니다.
...
### 절대 경로
절대 경로로 선언된 모듈의 경우, 루트 디렉토리부터 탐색해 리졸브해요.
...
### 상대 경로
상대 경로로 정의된 경우, 현재 파일을 기준으로 모듈을 탐색해요.
아래 예시의 경우, `Layout.tsx`이 위치한 디렉토리인 `components`에서 `Header.tsx`를 찾아요.
...
모듈 리졸루션은 단순히 파일 경로를 찾는 것으로 끝나지 않아요.  
번들러는 리졸루션 결과를 **전역 모듈 맵(Module Map)**에 저장해서, 동일 모듈에 대한 중복 탐색을 줄이고, 순환 참조에도 안정적으로 대응할 수 있도록 해요.
