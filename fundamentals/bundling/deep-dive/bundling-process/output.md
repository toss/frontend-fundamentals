...
번들 파일과 청크(chunk) 파일의 이름을 동적으로 설정하면 애플리케이션의 성능과 유지 보수에 효과적이에요. 특히 코드 스플리팅과 브라우저 캐시 관리를 쉽게 만들어 줘요.

고정된 파일 이름은 파일 내용이 변경되었을 때 브라우저가 이를 인지하지 못하고, 기존 캐시를 사용할 가능성이 높아요. 이런 상황이 발생하면 사용자는 오래된 버전의 애플리케이션을 보게 되고 최신 업데이트 내용을 적용받지 못하는 문제가 생겨요.

이를 방지하기 위해 파일 이름에 동적이고 고유한 값을 포함해, 파일이 변경될 때마다 새로운 이름을 부여할 수 있어요.

### 코드 스플리팅

파일 이름 설정에 `[id]`, `[name]` 같은 플레이스홀더를 사용하면 각 청크를 쉽게 구분할 수 있어 관리가 편해져요.
...
브라우저는 동일한 이름의 파일을 캐시해서 다시 로드하지 않기 때문에, 파일 내용이 바뀌어도 사용자는 이전 버전을 볼 수 있어요. 이를 해결하기 위해 파일 이름에 파일의 내용에 따라 달라지는 고유한 해시(hash) 값을 붙일 수 있어요.

- `[contenthash]`: 파일의 내용이 바뀔 때마다 자동으로 변경되는 해시 값
- `[chunkhash]`: 개별 청크 단위로 생성되는 고유한 해시 값
