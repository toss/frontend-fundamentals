...
진입점은 하나로 지정할 수도 있고, 여러 개로 나누어 지정할 수도 있어요. 
...
## 다음 단계
진입점을 기준으로 탐색을 시작한 번들러는 이제 각 모듈을 하나씩 따라가면서 실제 파일 경로를 찾아야 해요.
이 과정을 **모듈 리졸루션(Module Resolution)**이라고 해요.
다음 문서에서는 모듈 리졸루션이 어떻게 작동하는지, 그리고 리졸루션 과정을 통해 의존성 그래프가 점진적으로 확장되는 방식을 자세히 알아볼게요.