...
진입점(Entry Point)은 번들러가 애플리케이션을 분석하기 시작하는 최초의 파일이에요. 번들러는 이 파일을 기준으로 애플리케이션 내 모든 파일과 의존 관계를 추적하여 **실행 가능한 번들**을 생성해요.

진입점 설정 방식에는 크게 단일 엔트리와 다중 엔트리(객체 구문) 방식이 있어요. 각 방식의 차이는 생성되는 번들 파일의 수와 관리 방식이에요.
